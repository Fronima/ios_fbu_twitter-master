...
class APIManager: SessionManager {
    ...
    static let consumerKey = "RhXiEIeReeN3mJntD2WeUOVgK"
    static let consumerSecret = "3FoOQd5vxc8EWsrxJPIkC9MmGTd0BBOLLs8WA6PNbWuAlgoiXf"
    ...
    fileprivate func post(urlString: String, parameters: [String: Any], completion: @escaping (Tweet?, Error?) -> ()){
        
        request(urlString, method: .post, parameters: parameters, encoding: URLEncoding.queryString).validate().responseJSON{
            (response) in
    ...
    func favorite(_ tweet: Tweet, completion: @escaping (Tweet?, Error?) -> ()){
        ...
        let parameters = ["id": tweet.id]
            post(urlString: urlString, parameters: parameters, completion: completion)
    }
